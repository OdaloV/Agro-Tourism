...
export default function LocationPicker({ onLocationSelect, initialLocation }: LocationPickerProps) {
  ...
  const mapErrorHandled = useRef(false);
  ...
  const getDetailedAddress = async (lat: number, lng: number) => {
    try {
      const response = await fetch(
        ...
        { headers: { "User-Agent": "HarvestHost/1.0 (https://harvesthost.com)" } }
      );
      ...
      if (data?.address) {
        ...
        let detailedAddress = [neighborhood, ward, city, county].filter(Boolean).join(", ");
        if (!detailedAddress) detailedAddress = data.display_name.split(",")[0];
        if (!detailedAddress.toLowerCase().includes("kenya")) detailedAddress += ", Kenya";
        return { fullAddress: detailedAddress, neighborhood, ward, city, county, constituency };
      }
    } catch (err) {
    ...
    return {
      ...
      neighborhood: "", ward: "", city: "", county: "", constituency: "",
    };
  };
  ...
  const searchLocation = async (query: string) => {
    ...
    try {
      const response = await fetch(
        `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(query)}&countrycodes=ke&limit=10&addressdetails=1&zoom=18`,
        { headers: { "User-Agent": "HarvestHost/1.0 (https://harvesthost.com)" } }
      );
      const data = await response.json();
  ...
  const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    debounceTimeout.current = setTimeout(() => searchLocation(value), 500);
  };
  ...
  const selectSearchResult = async (result: any) => {
    ...
    setDetailedLocation({ neighborhood, ward, city, county, constituency: "", fullAddress: detailedAddress });

    if (map.current) {
      map.current.flyTo({ center: [lon, lat], zoom: 16 });
      marker.current?.setLngLat([lon, lat]);
      onLocationSelect({ lat, lng: lon, address: detailedAddress, city, region: county, country: "Kenya", district: ward, ward: neighborhood });
    }
  };
  ...
  useEffect(() => {
    ...
    function initMap() {
      ...
      map.current.on("load", () => {
        ...
        el.innerHTML = `<div style="width:32px;height:32px;background:#EAB308;border:3px solid white;border-radius:50%;box-shadow:0 2px 8px rgba(0,0,0,0.35);cursor:pointer;"></div>`;

        marker.current = new maplibregl.Marker({ draggable: true, element: el })
        ...
        if (initialLocation?.address) {
          ...
          onLocationSelect({
            ...
            city: "", region: "", country: "Kenya",
          });
        }
      });

      map.current.on("error", (e: any) => {
        if (mapErrorHandled.current) return;
        const msg: string = e?.error?.message || "";
        if (msg.includes("sprite") || msg.includes("glyph") || msg.includes("404")) return;
        mapErrorHandled.current = true;
        console.error("Map error:", msg || JSON.stringify(e));
        if (!mapLoaded) setError("Map failed to load. Check your connection and try refreshing.");
      });
    }
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        ...
        const messages: Record<number, string> = {
          1: "Please allow location access in your browser.",
          2: "Location information is unavailable.",
          3: "Location request timed out.",
        };
        alert(`Unable to get your location. ${messages[err.code] || ""}`);
        setGeolocating(false);
      },
    ...
    <div className="space-y-4">
          ...
          {showResults && searchResults.length > 0 && (
            <div className="absolute z-20 mt-1 w-full bg-white border border-gray-200 rounded-xl shadow-lg max-h-60 overflow-y-auto">
              {searchResults.map((result, idx) => {
                ...
                const city = addr.city || addr.town || addr.village || "";
                const county = addr.state || addr.county || "";
                const displayName = neighborhood || ward || city || county || result.display_name.split(",")[0];
                const subText = [ward !== displayName ? ward : "", city !== displayName ? city : "", county].filter(Boolean).join(", ");
                return (
                  <button key={idx} type="button" onClick={() => selectSearchResult(result)}
      ...
      <div>
        ...
        <div className="flex gap-2">
          ...
          <button type="button" onClick={getCurrentLocation} disabled={geolocating}
            ...
            {geolocating
              ...
              : <><span>🎯</span><span className="hidden sm:inline">Current</span></>}
          </button>
        </div>
      </div>
      ...
      <div ref={mapContainer} className="w-full rounded-xl overflow-hidden border border-gray-300 shadow-sm"
