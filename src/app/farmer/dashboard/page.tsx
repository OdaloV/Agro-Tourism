  ...
  useEffect(() => {
    ...
    const fetchFarmerData = async () => {
      try {
        ...
        } catch (err) { console.error(err); data.farmPhotos = 0; }
      ...
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/auth/register/farmer">
            <button className="mt-4 px-6 py-2 bg-emerald-600 text-white rounded-xl">Complete Profile</button>
          </Link>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-emerald-100/30">
      <div className="container mx-auto px-4 py-8 max-w-6xl">
        ...
        {/* Header - No logout button, only ThemeToggle */}
        <div className="mb-8">
          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className="w-48 h-48 rounded-full bg-gradient-to-br from-emerald-500 to-emerald-700 flex items-center justify-center text-white text-lg font-bold overflow-hidden flex-shrink-0">
                  {profilePhoto ? (
                  ...
                  <button 
                    onClick={async () => {
                      try {
                        ...
                        if (response.ok) alert("Verification email resent! Check your inbox.");
                        else alert("Failed to resend. Please try again.");
        ...
        {/* Main Content Grid - only farm info and media gallery */}
        ...
        {/* Verification status card for pending/rejected only (shown below both columns) */}
